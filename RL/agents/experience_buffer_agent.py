...
class ExperienceBufferAgent(RL.Agent):
    def __init__(self, context: RL.Context, name, nsteps=1, buffer_length=1e6, buffer_size_MB=None):
        super().__init__(context, name)
        self.nsteps = nsteps
        if buffer_length is None:
            self.experience_buffer = ExperienceBuffer(size_in_bytes=buffer_size_MB * (1024**2))
        else:
            self.experience_buffer = ExperienceBuffer(length=buffer_length)
