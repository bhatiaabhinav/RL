...
class BasicStatsRecordingAgent(RL.Agent):
    def __init__(self, context: RL.Context, name, frameskip=None, record_returns=None, record_unscaled_rewards=None, reward_scaling=None, safety_reward_scaling=None):
        ...
        self.safety_reward_scaling = context.safety_reward_scaling if safety_reward_scaling is None else safety_reward_scaling
        self._reward_unscaling = 1 / self.reward_scaling if self.record_unscaled_rewards else 1
        self._safety_reward_unscaling = 1 / self.safety_reward_scaling if self.record_unscaled_rewards else 1
    ...
    def post_act(self):
        ...
        if self.record_returns:
            self.episode_safety_rewards += (self.context.safety_gamma ** self.runner.episode_step_ids) * safety_rewards * self._safety_reward_unscaling
        else:
            self.episode_safety_rewards += safety_rewards * self._safety_reward_unscaling
