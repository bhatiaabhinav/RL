...
class GatherViewer(MjViewer):
    def __init__(self, env):
        ...
        red_ball_highlighted_model = MjModel(osp.abspath(
            osp.join(
                MODEL_DIR, 'red_ball_highlighted.xml'
            )
        ))
        ...
        for obj in self.env.objects_in_view:
            ...
            if typ == APPLE:
                ...
                mjextra.append_objects(
                    tmpobjects, self.green_ball_highlighted_renderer.objects)
            else:
...
class GatherEnv(Env, Serializable):
    ...
    def get_body_com(self, body_name):
        idx = self.inner_env.model.body_names.index(body_name)
        return self.inner_env.model.data.com_subtree[idx]

    @overrides
    def log_diagnostics(self, paths):
        apples_collected = [path['env_infos']['apples'].sum() for path in paths]
        bombs_collected = [path['env_infos']['bombs'].sum() for path in paths]
