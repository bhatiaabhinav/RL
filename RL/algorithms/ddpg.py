...
from RL.agents import SeedingAgent, TensorFlowAgent, DDPGActAgent, ParamNoiseAgent, ExperienceBufferAgent, DDPGTrainAgent, ParamsCopyAgent, ModelLoaderSaverAgent, EnvRenderingAgent, DQNSensitivityVisualizerAgent, PygletLoopAgent, BasicStatsRecordingAgent, StatsLoggingAgent  # noqa: F401
import gym
...
r = RL.Runner(c, "runner", 2000, num_episodes_to_run=c.n_episodes)
...
r.register_agent(StatsLoggingAgent(c, "StatsLoggingAgent", ['Ep Id', 'Ep R', 'Av Ep R (100)', 'Ep L', 'Total Steps', 'Ep Exploit Mode', 'Av Exploit Ep R (100)', 'Total Episodes'], poll_every_episode=10, keys_prefix='Env-0 '))
# r.register_agent(StatsLoggingAgent(c, "MiscLoggingAgent", ['epsilon', 'DQNActAgent/main_brain/default/Q_loss'], poll_every_episode=10))
if c.render:
